...
// HasActionInstance returns true if and only if the set contains the action
// instance with the given address.
func (s Set) HasActionInstance(want addrs.AbsActionInstance) bool {
	return s.exp.knowsActionInstance(want)
}

// HasAction returns true if and only if the set contains the action with
// the given address, even if that actions has no instances.
